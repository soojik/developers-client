...
const AwsS3: React.FC<S3Select> = ({ s3select, uploadCheck }) => {
  ...
  const handleFileInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      }-${currentDate.getDate()}_${currentDate.getHours()}-${currentDate.getMinutes()}`;
  ...
  return (
    <div>
      <div className="rounded border border-gray-400 p-4">
        ...
        <input
          className="py-1 px-1 bg-transparent  font-semibold border border-gray-600 rounded hover:bg-gray-400 hover:text-white hover:border-transparent transition ease-in duration-200 transform hover:-translate-y-1 text-xs"
          type="file"
