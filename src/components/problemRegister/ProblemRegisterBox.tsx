...
const handleDeleteHashTagClick=(i:Number,item:string)=>{
  ...
  return (
    ...
    <PointBox point={point} handlePointChange={handlePointChange}/>
      ...
      {isHashTag && <div className="flex ">{hashTag.map((item,index)=><div >
<div className="ml-5 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-5">{item}
<button onClick={()=>{
handleDeleteHashTagClick(index+1,item);
      ...
      {isSubjective ? (
        <SubjectiveAnswer children={subjectiveValue} onChange={(e) => {
              SetSubjectiveValue(e.target.value);
        }} />
      ) : (
        <ObjectiveAnswer answers={answers} setAnswers={setAnswers} Count={4} onChange={() => {}} />
      )}
      <div>
