...
const MainList = ({ section, sectionHeader }: Type) => {
  ...
  const loadMoreData = async () => {
    ...
    const newData = [
      {
        nickname: `유저${page * 5 + 1}`,
        type: "🔢객관식",
        views: 0,
        title: `제목입니다${page * 5 + 1}`,
        likes: 0,
      },
      {
        nickname: `유저${page * 5 + 2}`,
        type: "✍️단답형",
        views: 0,
        title: `제목입니다${page * 5 + 2}`,
        likes: 0,
      },
      {
        nickname: `유저${page * 5 + 3}`,
        type: "🔢객관식",
        views: 0,
        title: `제목입니다${page * 5 + 3}`,
        likes: 0,
      },
      {
        nickname: `유저${page * 5 + 4}`,
        type: "✍️단답형",
        views: 0,
        title: `제목입니다${page * 5 + 4}`,
        likes: 0,
      },
      {
        nickname: `유저${page * 5 + 5}`,
        type: "🔢객관식",
        views: 0,
        title: `제목입니다${page * 5 + 5}`,
        likes: 0,
      },
    ];
    ...
    <>
      ...
      >
        {data.map((el) => (
          <div
            key={el.title}
            className="relative group hover:text-slate-300 cursor-pointer bg-gray-200 rounded-lg"
          ...
          >
            ...
            <div className="relative z-10 bg-gray-200 w-full h-full transition-transform transform ease duration-100 group-hover:translate-x-2 group-hover:-translate-y-2 rounded-lg">
              ...
              <div className="flex justify-between">
                ...
                <div className="flex">
                  <div className="text-sm mr-4 text-black">
                    <label className="font-bold">조회수</label>
                    {el.views}
                  </div>
                  <div className="text-sm mr-4 text-black">
                    <label className="font-bold">좋아요</label>
                    {el.likes}
                  </div>
                </div>
              </div>
