...
const ProblemDetail = () => {
  ...
  const handleLikeButtonClick = async () => {
    try {
      const response = await axiosInstance.post(`http://localhost:9001/api/problem/${location.state.problemId}`);
      setIsLiked(true);
      setLikes(likes + 1);
    } catch (error) {
      if (axios.isAxiosError(error)) {
    ...
    <>
      ...
      <div className=" bg-gray-200 px-20 py-10 rounded">
        <div className="mt-4">
          ...
          <div className="mt-4">
            <p className="text-lg text-gray-500 bg-white rounded py-9">{detail.content}</p>
          </div>
